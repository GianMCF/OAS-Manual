...
const positions = [ "Administrador", "Jefe Comercial", "Supervisor de Ventas", "Vendedor", "Asistente de Ventas", "Encargado de Almacén", "Logística", "Atención al Cliente" ]

const salaries = { "Administrador": 3500, "Jefe Comercial": 4200, "Supervisor de Ventas": 3000, "Vendedor": 1800, "Asistente de Ventas": 1500, "Encargado de Almacén": 1900, "Logística": 2200, "Atención al Cliente": 1600 }

const wineNames = [ "Vino Tinto Reserva", "Vino Rosé Premium", "Vino Blanco Seco", "Vino Gran Reserva", "Vino Espumante" ]

const wineries = [ "Tabernero", "Tacama", "Santiago Queirolo", "Intipalka", "Viñas de Oro", "La Caravedo", "Don Santiago", "Bodega San Nicolás" ]

const wineTypes = [ "Tinto", "Blanco", "Rosado", "Espumante" ]

const grapeVarieties = [ "Malbec", "Cabernet Sauvignon", "Merlot", "Syrah", "Chardonnay", "Sauvignon Blanc", "Quebranta", "Italia", "Torontel", "Moscatel" ]

const productLines = [ "Reserva", "Gran Reserva", "Premium", "Selección Especial", "Colección", "Estate", "Clásico", "Edición Limitada" ]

const piscoTypes = [ "Pisco Quebranta", "Pisco Italia", "Pisco Torontel", "Pisco Acholado", "Pisco Mosto Verde" ]
...
for(let i = 1; i <= 100; i++) {
  ...
  if(isWine){
    ...
    const winery =
      wineries[
        Math.floor(Math.random() * wineries.length)
      ]

    const type =
      wineTypes[
        Math.floor(Math.random() * wineTypes.length)
      ]

    const grape =
      grapeVarieties[
        Math.floor(Math.random() * grapeVarieties.length)
      ]

    const line =
      productLines[
        Math.floor(Math.random() * productLines.length)
      ]
    ...
    const pisco =
      piscoTypes[
        Math.floor(Math.random() * piscoTypes.length)
      ]
...
const clients =
  db.client.find({ status: true }).toArray()

const employees =
  db.employee.find({ status: true }).toArray()

const products =
  db.products.find({ status: true }).toArray()

const paymentMethods =
  db.payment_method.find({ status: true }).toArray()
